...
const insulateData = {
 ...
 let calc_e6;
 let calc_e7;
 let calc_e8;//товщина слоя 
 let calc_e9;// теплопровідність обраного шару
 let layer_e8_e9=0;//розрахунок шарів
 let calc_e10;



 let calc_e14 = 8.7; //Коэффициент теплоотдачи внутренней поверхности ограждающей конструкции
 let calc_e15 = 23;
 ...
 btnCalculate.addEventListener('click',function(e){
   e.preventDefault();
      ...
      let calc_e11=(insulateData[insulatSel.value][insulatSel.value-1].values.calc_e11);
      let calc_e7_param_1=(insulateData[insulatSel.value][insulatSel.value-1].values.calc_e7_param_1);
      let calc_e7_param_2=(insulateData[insulatSel.value][insulatSel.value-1].values.calc_e7_param_2);

      
      let calc_e3 = temper.value;//Температура всередині об'єкта 

      ////зчитуємо значення з регіонів
      const selectedOption = regionSelect.options[regionSelect.selectedIndex];
      let calc_e4 = selectedOption.dataset.temperature;
      let calc_e5 = selectedOption.dataset.days;
      ...
      materials.forEach((material, index) => {
         calc_e8 = thicknesses[index].value; 
         calc_e9 = material.value;
   ...
   calc_e10 = calc_e10.toFixed(4); 

   real_material_consumption = actual_foam_density * cacl_area_of_application * calc_e10 * 100 * 0.01 * 1.08;

   // result.textContent=`Витрата матеріалу: ${real_material_consumption}`;

   result.textContent=`Товщина теплоізоляційного шару огороджувальної конструкції: ${calc_e10} м
